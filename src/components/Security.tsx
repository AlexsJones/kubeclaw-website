const threats = [
  {
    ...
    kubeclaw: 'Each agent runs in an ephemeral Pod with deny-all egress NetworkPolicy. Blast radius = one Job.',
    icon: (
  ...
  },
  {
    ...
    kubeclaw: 'Ephemeral RBAC per AgentRun — controller provisions least-privilege credentials at start, revokes on completion. No standing god-roles.',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    kubeclaw: 'NetworkPolicy deny-all egress on every agent pod. Only the IPC bridge sidecar can reach NATS. Agents cannot reach the internet or other pods.',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    kubeclaw: 'ClawPolicy CRD + admission webhook — tools and features gated before the pod is even created. Enforcement at the API server, not in your app.',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    kubeclaw: 'ownerReference garbage collection for namespace RBAC. Label-based cleanup for cluster RBAC. Everything is deleted when the AgentRun finishes.',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    kubeclaw: 'Namespace-scoped CRDs with Kubernetes RBAC. Each tenant gets their own namespace, policies, and network boundaries.',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const layers = [
  ...
  },
  {
    ...
    detail: 'ClawPolicy webhook enforced before pod creation',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="text-center mb-20">
          ...
          <p className="text-lg text-slate-400 max-w-3xl mx-auto">
            ...
            KubeClaw enforces <span className="text-white font-semibold">defence-in-depth at every layer</span> — from
        ...
        <div className="mb-16">
          ...
          <p className="text-slate-500 text-center mb-10 max-w-2xl mx-auto">
            Every security concern you have about running AI agents in production — KubeClaw addresses it at the infrastructure level, not with application-level workarounds.
          </p>
              ...
              >
                ...
                <div className="px-6 pb-6 space-y-3">
                  ...
                  <div className="flex gap-3 p-3 rounded-lg bg-claw-green/5 border border-claw-green/10">
                    ...
                    <div>
                      <span className="text-xs font-medium text-claw-green/70 uppercase tracking-wider">KubeClaw</span>
                      <p className="text-sm text-slate-300 mt-0.5">{t.kubeclaw}</p>
                    </div>
                  </div>
                </div>
              </div>
